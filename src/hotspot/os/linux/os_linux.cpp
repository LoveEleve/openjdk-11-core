...
#define EXTENSIONS_DIR  "/lib/ext"
    ...
    const size_t bufsize =
            MAX2((size_t) MAXPATHLEN,  // For dll_dir & friends.
                 (size_t) MAXPATHLEN + sizeof(EXTENSIONS_DIR) + sizeof(SYS_EXT_DIR) +
                 sizeof(EXTENSIONS_DIR)); // extensions dir
...
static void *thread_native_entry(Thread *thread) { // 这里传入的是JavaThread对象(jvm内部的)
  ...
  *(char *)stackmem = 1;
    ...
    thread->initialize_thread_current(); // 初始化TLS(在任何地方都能通过Thread::current()获取当前线程)

    OSThread *osthread = thread->osthread(); // 获取OSThread对象(这在之前是保存过的)
    Monitor *sync = osthread->startThread_lock();

    osthread->set_thread_id(os::current_thread_id()); // 设置线程id
    ...
    // handshaking with parent thread
    {
        ...
        osthread->set_state(INITIALIZED); // 设置osthread对象的状态 - 由 ALLOCATED -> INITIALIZED
        // sync_with_child->wait(Mutex::_no_safepoint_check_flag); 之前父线程阻塞在这里,等待子线程初始化完成
        sync->notify_all(); // 唤醒父线程

        // wait until os::start_thread()
        // 在这里子线程阻塞等待父线程调用os::start_thread()
...
bool os::create_thread(Thread *thread, ThreadType thr_type,
    ...
    // 将osthread 关联到 JavaThread上
    thread->set_osthread(osthread);

    // init thread attributes
    pthread_attr_t attr;
    pthread_attr_init(&attr);
    // 设置线程属性：PTHREAD_CREATE_DETACHED,这个作用是什么呢？
...

    {
        ...
        do {
            /*
                参数:
                    ...
                    - void *(*start_routine) (void *): 线程执行函数{接受void*参数,返回值也是void*} - thread_native_entry
                    - void *arg:传递给start_routine的,在这里将JavaThread对象作为参数传递给回调方法
            */
            ret = pthread_create(&tid, &attr, (void *(*)(void *)) thread_native_entry, thread);
        } while (ret == EAGAIN && limit-- > 0);

        char buf[64];
        if (ret == 0) { // pthread_create()成功
            log_info(os, thread)("Thread \"%s\" started (pthread id: " UINTX_FORMAT ", attributes: %s). ",
                                 thread->name(), (uintx) tid,
                                 os::Posix::describe_pthread_attr(buf, sizeof(buf), &attr));
        } else {
            log_warning(os, thread)("Failed to start thread \"%s\" - pthread_create failed (%s) for attributes: %s.",
                                    thread->name(), os::errno_name(ret),
                                    os::Posix::describe_pthread_attr(buf, sizeof(buf), &attr));
        ...
        // 等待新线程初始化完毕
        {
            ...
            // 等待新线程状态变为INITIALIZED(此时父线程阻塞在这里,在当前情况中是main线程)
            // 到这里就需要先去看下 thread_native_entry 方法了,因为这是新线程真正的执行入口
...
void os::pd_start_thread(Thread *thread) {
    ...
    Monitor *sync_with_child = osthread->startThread_lock();
    MutexLockerEx ml(sync_with_child, Mutex::_no_safepoint_check_flag);
    sync_with_child->notify(); // 唤醒子线程,在这里和前面呼应起来了,下面就要回到 thread_native_entry()方法中了
}
...
#define _DFM INTX_FORMAT
                ...
                i = sscanf(s,
                           "%c %d %d %d %d %d %lu %lu %lu %lu %lu %lu %lu %ld %ld %ld %ld %ld %ld " _UFM _UFM _DFM _UFM _UFM _UFM _UFM,
                           &state,          // 3  %c
                           &ppid,           // 4  %d
                           &pgrp,           // 5  %d
                           &session,        // 6  %d
...
static void print_sys_devices_cpu_info(outputStream *st, char *buf, size_t buflen) {
    ...
    _print_ascii_file_h("Frequency switch latency (ns)",
                        "/sys/devices/system/cpu/cpu0/cpufreq/cpuinfo_transition_latency", st);
    _print_ascii_file_h("Available cpu frequencies",
                        "/sys/devices/system/cpu/cpu0/cpufreq/scaling_available_frequencies", st);
    ...
    if (ExtensiveErrorReports) {
        _print_ascii_file_h("Available governors", "/sys/devices/system/cpu/cpu0/cpufreq/scaling_available_governors",
                            st);
    }
...
static bool linux_mprotect(char *addr, size_t size, int prot) {
    ...
    Events::log(NULL, "Protecting memory [" INTPTR_FORMAT "," INTPTR_FORMAT "] with protection modes %x", p2i(bottom),
...
// forcus 初始化 Linux系统的大页支持
void os::large_page_init() {
    // 如果所有大页相关的标志都未启用，直接返回，不进行大页初始化
    if (!UseLargePages &&
    ...
    /*
     * 如果用户通过命令行参数显式设置了 -XX:-UseLargePages（关闭大页），则：
     *  - 忽略其他所有大页相关的标志 / 将所有大页类型的标志都设置为 false / 返回
     */
     ...
     *      - 从 /proc/meminfo 读取系统支持的大页大小（通常是 2MB 或 1GB）
     *      - 将大页大小保存到 _large_page_size 变量
     *      - 更新 _page_sizes 数组，包含大页和默认页大小
     */
    size_t large_page_size = Linux::setup_large_page_size();
    // forcus 设置大页类型
    /*
     *  调用 Linux::setup_large_page_type() 方法来确定使用哪种大页实现方式：
     *   - UseHugeTLBFS: 使用 HugeTLBFS 文件系统（通过 mmap 的 MAP_HUGETLB 标志）
     *   - UseSHM: 使用共享内存（通过 shmget 的 SHM_HUGETLB 标志）
     *   - UseTransparentHugePages: 使用透明大页（THP，内核自动管理）
     */
    UseLargePages = Linux::setup_large_page_type(large_page_size);
    // forcus 设置 coredump 过滤器
    // 设置 /proc/self/coredump_filter，使得在生成 core dump 文件时包含大页内存区域，这对于调试很有帮助。
