...
void GCConfig::select_gc_ergonomically() {
  if (os::is_server_class_machine()) { // forcus 服务级别 - G1
#if INCLUDE_G1GC
...
#if INCLUDE_SERIALGC
    FLAG_SET_ERGO_IF_DEFAULT(bool, UseSerialGC, true); // 客户端级别 - SerialGC
#endif
  }
}
...
GCArguments* GCConfig::select_gc() {
  ...
  // forcus 检查是否选择了不支持的GC
  fail_if_unsupported_gc_is_selected();
  // forcus 如果用户没有指定任何的GC
  ...
  // forcus 检查是否只选择了一个gc，如果选择了多个则报错
