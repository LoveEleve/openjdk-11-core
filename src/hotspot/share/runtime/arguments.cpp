...
void set_object_alignment() {
  // Object alignment.
  // forcus 默认值为8字节 - MinObjAlignmentInBytes - 对象的最小对齐字节数 {所有java对象在堆上分配时,起始地址必须是 MinObjAlignmentInBytes 字节对齐的}
  assert(is_power_of_2(ObjectAlignmentInBytes), "ObjectAlignmentInBytes must be power of 2");
  MinObjAlignmentInBytes     = ObjectAlignmentInBytes;
  // forcus 对象的最小对齐值，以 HeapWord 为单位 (在 GC 和内存分配时，以 HeapWord 为单位计算更高效)
  assert(MinObjAlignmentInBytes >= HeapWordsPerLong * HeapWordSize, "ObjectAlignmentInBytes value is too small");
  MinObjAlignment            = MinObjAlignmentInBytes / HeapWordSize;
  // forcus  设置对齐掩码 - 用于快速判断地址是否对齐的位掩码
  // (address & MinObjAlignmentInBytesMask) == 0 表示地址已对齐
  assert(MinObjAlignmentInBytes == MinObjAlignment * HeapWordSize, "ObjectAlignmentInBytes value is incorrect");
  MinObjAlignmentInBytesMask = MinObjAlignmentInBytes - 1;
  // 设置对数值 - 对齐值的对数，用于位移运算 - 将乘除法转换为更高效的位移操作
  ...
  // forcus 计算压缩指针最大堆大小 -> (2³² ) << 3 = 4GB << 3 = 32GB
  //
  /*
   * 使用压缩指针 (Compressed Oops) 时支持的最大堆大小
   *  - 这行计算是什么意思？- jvm在使用压缩指针时,能够寻址的最大堆内存大小
   *   - 使用了压缩指针后,只需要4B(32)位来存储对象引用,而32位只能表示 2³² = 4GB 的地址空间
   *   - 由于所有的对象都按照 8B 对齐,因此对象地址的低3位永远是0(对齐的要求),
   *   - 因此可以省略这3位,用于扩展寻址范围 - 也即最终在开启压缩指针的情况下,,能够寻址的最大堆内存大小 = 32GB(4GB * 2^3)
   *    - 什么意思呢？此时内存地址不再是具体的的某一个位置,而是某一个“格子”(一个格子8字节 - 因为是8字节对齐)
   *    - 所以能访问的内存地址变为了32GB
   */
  OopEncodingHeapMax = (uint64_t(max_juint) + 1) << LogMinObjAlignmentInBytes;
  // forcus 设置 Survivor 区对齐
  // 如果用户没有显式设置 Survivor 区对齐，则使用对象对齐值 , 控制年轻代 Survivor 区中对象的对齐方式
...
jint Arguments::parse_vm_init_args(const JavaVMInitArgs *vm_options_args, // java.base模块的vm选项
                                   const JavaVMInitArgs *java_tool_options_args, // JAVA_TOOL_OPTIONS环境变量
                                   const JavaVMInitArgs *java_options_args, // _JAVA_OPTIONS环境变量
                                   const JavaVMInitArgs *cmd_line_args) { // 命令行参数
  bool patch_mod_javabase = false;

  // Save default settings for some mode flags
  // forcus 保存JIT编译相关的默认参数
  // =====
  // 是否总是编译包含循环的方法(这是一种激进的编译策略) - 默认为false,也即遵循正常的调用计数阈值
  Arguments::_AlwaysCompileLoopMethods = AlwaysCompileLoopMethods; // false
  // forcus 是否启用栈上替换(OSR)
  /*
   * OSR介绍：
   *    - 当一个方法正在解释执行时，如果其中的循环被执行了很多次（达到 OnStackReplacePercentage 阈值），JVM 会在循环执行过程中将解释执行切换为编译后的本地代码执行
   *    - 不需要等待方法退出再重新进入，可以在循环内部直接"替换"执行方式
   */
  Arguments::_UseOnStackReplacement    = UseOnStackReplacement; // true
  // forcus : 控制是否限制内联深度
  /*
   * 当设置为true时,JIT编译器会限制方法内联的深度和大小,防止过度内联导致编译后的代码膨胀
   *  - 内联：将被调用方法的代码直接嵌入到调用者中，消除代码调用的开销
   */
  Arguments::_ClipInlining             = ClipInlining; // true
  // forcus 是否开启后台编译
  /*
   * JIT编译在后台线程中异步执行,不会阻塞主线程
   */
  Arguments::_BackgroundCompilation    = BackgroundCompilation; // true
  // 分层编译相关参数
  if (TieredCompilation) {
  ...
  // forcus:设置混合模式(默认的,但是我使用了-Xint来修改为解释执行了)
  ...
  // forcus === 按照优先级顺序解析参数 -- java.base 模块的内置VM选项 -> JAVA_TOOL_OPTIONS环境变量 -> 命令行参数 -> _JAVA_OPTIONS环境变量(优先级最高,可以覆盖前面的参数)
