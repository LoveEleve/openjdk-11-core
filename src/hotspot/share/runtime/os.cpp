...
void os::init_before_ergo() {
  // forcus 初始化活跃处理器数量 -> 保存到全局变量 _initial_active_processor_count
  initialize_initial_active_processor_count();
  // We need to initialize large page support here because ergonomics takes some
  // decisions depending on large page support and the calculated large page size.
  // forcus 初始化大页内存支持(Large Page 可以减少TLB miss)
  large_page_init();
...
void os::start_thread(Thread* thread) {
  ...
  OSThread* osthread = thread->osthread(); // 获取OSThread对象
  osthread->set_state(RUNNABLE); // 将OSThread状态设置为RUNNABLE(ALLOCATED - INITIALIZED - RUNNABLE)
...
bool os::is_server_class_machine() {
  ...
  // forcus-1 两个参数(但是默认都是false)
  if (NeverActAsServerClassMachine) { // false - 当前机器永远不当作server机器
    return false;
  }
  if (AlwaysActAsServerClassMachine) { // false - 当前机器永远当作server机器
    return true;
  }
  // Then actually look at the machine
  bool         result            = false;
  // forcus CPU ≥ 2  且 内存 ≥ 2GB
  const unsigned int    server_processors = 2; // 2个CPU
  const julong server_memory     = 2UL * G; // 2G内存
  // We seem not to get our full complement of memory.
  //     We allow some part (1/8?) of the memory to be "missing",
  //     based on the sizes of DIMMs, and maybe graphics cards.
  const julong missing_memory   = 256UL * M; // 允许丢失的内存 256M

  /* Is this a server class machine? */
  // forcus 检查当前机器真正的硬件配置
  // forcus-1 CPU ≥ 2 且 内存 ≥ (2GB - 256M)
