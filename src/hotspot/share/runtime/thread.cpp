...
THREAD_LOCAL_DECL Thread
*
Thread::_thr_current = NULL;
#endif
...
void *Thread::allocate(size_t size, bool throw_excpt, MEMFLAGS flags) {
    if (UseBiasedLocking) {
        ...
        if (aligned_addr != real_malloc_addr) {
            log_info(biasedlocking)("Aligned thread "
            INTPTR_FORMAT
            " to "
            INTPTR_FORMAT,
        ...
        ((Thread * )
        aligned_addr)->_real_malloc_address = real_malloc_addr;
        return aligned_addr;
    } else {
...
Thread::Thread() {
    ...
    if (UseBiasedLocking) {
        assert((((uintptr_t)
        this) &(markOopDesc::biased_lock_alignment - 1)) == 0,
                "forced alignment of thread object failed");
...
void Thread::initialize_thread_current() {
    ...
    _thr_current = this; // forcus this 指定的是当前JavaThread对象
#endif
    assert(ThreadLocalStorage::thread() == NULL, "ThreadLocalStorage::thread already initialized");
    ThreadLocalStorage::set_thread(this); // forcus threadLocal
...
void Thread::record_stack_base_and_size() {
    ...
    if (is_Java_thread()) {
        ((JavaThread * )
        this)->set_stack_overflow_limit(); // forcus 设置栈溢出边界(当栈指针低于这个指时,jvm就知道快要溢出了)
        ((JavaThread * )
        this)->set_reserved_stack_activation(stack_base()); // forcus 初始化为栈顶 stack_base(高地址)
    }
}
...
Thread::~Thread() {
    ...
    _SleepEvent = NULL;
    ParkEvent::Release(_MutexEvent);
    _MutexEvent = NULL;
    ParkEvent::Release(_MuxEvent);
    _MuxEvent = NULL;
    ...
    if (CheckUnhandledOops) delete unhandled_oops();)
}
...
void Thread::set_priority(Thread * thread, ThreadPriority
priority) {
...
// Can return an error!
(void)
os::set_priority(thread, priority
);
}

// 启动一个已经创建好的线程,让它真正开始运行
void Thread::start(Thread * thread) { // 这里的Thread，
    // Start is different from resume in that its safety is guaranteed by context or
    // being called from a Java method synchronized on the Thread object.
    if (!DisableStartThread) {
        if (thread->is_Java_thread()) { // 是否是java_thread,在jvm中的JavaThread默认会返回true
            // Initialize the thread state to RUNNABLE before starting this thread.
            // Can not set it after the thread started because we do not know the
            // exact thread state at that time. It could be in MONITOR_WAIT or
            // in SLEEPING or some other state.
            // 在这里将java.lang.thread类的状态设置为RUNNABLE,必须在线程启动之前设置
...
bool Thread::is_interrupted(Thread * thread, bool
clear_interrupted) {
debug_only(check_for_dangling_thread_pointer(thread);
)
...
return
os::is_interrupted(thread, clear_interrupted
);
}
...
void Thread::print_on(outputStream *st, bool print_extended_info) const {
    ...
    if (osthread() != NULL) {
        ...
        if (is_Java_thread() && (PrintExtendedThreadInfo || print_extended_info)) {
            ...
            st->print("defined_classes="
            INT64_FORMAT
            " ", _statistical_info.getDefineClassCount());
        }
    ...
    st->print(" ");
    debug_only(
    if (WizardMode) print_owned_locks_on(st);)
}
    ...
    if (potential_vm_operation && is_Java_thread()
        ...
        for (Monitor *cur = _owned_locks; cur; cur = cur->next()) {
            ...
            if ((cur->allow_vm_block() &&
                 cur != Threads_lock &&
                 cur != Compile_lock &&
                 // Temporary: should not be necessary when we get separate compilation
...
bool Thread::set_as_starting_thread() {
    ...
    return os::create_main_thread((JavaThread * )
    this);
}
...
// 这个是java线程被唤醒后调用的第一个方法
void JavaThread::run() {
    // initialize thread-local alloc buffer related fields
    this->initialize_tlab(); // 初始化TLAB

    // used to test validity of stack trace backs
    this->record_base_of_stack_pointer(); // 记录栈基址指针,用于后续验证栈回溯的有效性

    this->create_stack_guard_pages(); // 创建栈保护页，防止栈溢出

    this->cache_global_variables(); // 缓存全局变量 将一些全局变量缓存到线程本地，提高访问效率

    // Thread is now sufficient initialized to be handled by the safepoint code as being
    // in the VM. Change thread state from _thread_new to _thread_in_vm
    // 将线程状态从 _thread_new 转换为 _thread_in_vm,注意此时是在哪个线程中？此时就是在java线程(不是主线程)
    ...
    // been completed.
    this->set_active_handles(JNIHandleBlock::allocate_block()); // 为线程分配 JNI 句柄块。
    ...
    // 我们调用另一个函数来完成剩余工作，这样可以确保从那里使用的栈地址低于刚计算的栈基址
    // 确保栈帧布局正确，后续的栈地址都在刚记录的栈基址之下
    thread_main_inner(); // 调用java代码,也就是在java层面写的run方法
...
void JavaThread::thread_main_inner() {
    ...
    // 确保线程没有待处理的异常 以及 被stop()
    if (!this->has_pending_exception() &&
        !java_lang_Thread::is_stillborn(this->threadObj())) {
        {
            ResourceMark rm(this);
            this->set_native_thread_name(this->get_thread_name()); // 设置线程名称(会反映到linux内核中)
        }
        HandleMark hm(this);
        this->entry_point()(this, this); // 调用entry_point,将当前JavaThread对象传入
    }
...
void JavaThread::exit(bool destroy_vm, ExitType exit_type) {
    ...
    log_info(os, thread)("JavaThread %s (tid: "
    UINTX_FORMAT
    ").",
    ...
    if (_pending_async_exception != NULL) {
        ...
        if (!has_pending_exception() || !pending_exception()->is_a(SystemDictionary::ThreadDeath_klass())) {
            ...
            if (lt.is_enabled()) {
                ResourceMark rm;
                LogStream ls(lt);
                ls.print("Async. exception installed at runtime exit ("
                INTPTR_FORMAT
                ")", p2i(this));
...
void JavaThread::send_thread_stop(oop java_throwable) {
...

    {
        ...
        if (_pending_async_exception == NULL ||
            !_pending_async_exception->is_a(SystemDictionary::ThreadDeath_klass())) {
...
int JavaThread::java_suspend_self() {
    ...
    assert(_anchor.walkable() ||
           (is_Java_thread() && !((JavaThread * )
    this)->has_last_Java_frame()),
...
void JavaThread::check_safepoint_and_suspend_for_native_trans(JavaThread *thread) {
    ...
    assert(!curJT->has_last_Java_frame() || curJT->frame_anchor()->walkable(),
           "Unwalkable stack in native->vm transition");
...
void JavaThread::metadata_do(void f(Metadata *)) {
    if (has_last_Java_frame()) {
        ...
        CompilerThread * ct = (CompilerThread * )
        this;
        if (ct->env() != NULL) {
...
const char *_get_thread_state_name(JavaThreadState _thread_state) {
    switch (_thread_state) {
        ...
        case _thread_new_trans:
            return "_thread_new_trans";
        case _thread_in_native:
            return "_thread_in_native";
        case _thread_in_native_trans:
            return "_thread_in_native_trans";
        case _thread_in_vm:
            return "_thread_in_vm";
        case _thread_in_vm_trans:
            return "_thread_in_vm_trans";
        case _thread_in_Java:
            return "_thread_in_Java";
        case _thread_in_Java_trans:
            return "_thread_in_Java_trans";
        case _thread_blocked:
            return "_thread_blocked";
        case _thread_blocked_trans:
            return "_thread_blocked_trans";
        default:
...
void JavaThread::print_on(outputStream *st, bool print_extended_info) const {
    ...
    if (thread_oop != NULL) {
        st->print("#"
        INT64_FORMAT
        " ", (int64_t) java_lang_Thread::thread_id(thread_oop));
...
#endif // PRODUCT
    if (is_Compiler_thread()) {
        CompileTask * task = ((CompilerThread * )
        this)->task();
        if (task != NULL) {
            st->print("   Compiling: ");
...
const char *JavaThread::get_threadgroup_name() const {
    debug_only(
    if (JavaThread::current() != this) assert_locked_or_safepoint(Threads_lock);)
    oop thread_obj = threadObj();
    if (thread_obj != NULL) {
...
// 将 Java 层的 Thread 对象与 C++ 层的 JavaThread 对象建立双向关联，并完成线程的最终初始化
void JavaThread::prepare(jobject jni_thread, ThreadPriority prio) {
    ...
    // 此时执行到这里的应该是父线程,这里的jthread是Java中的Thread(java层面的,这里涉及到JNI调用时的参数传递)
    // 这里是将JNI引用解析为oop,并且用Handle包装,这个thread_oop()就是 Java Thread对象在堆中的地址
    Handle thread_oop(Thread::current(),
                      JNIHandles::resolve_non_null(jni_thread));
    assert(InstanceKlass::cast(thread_oop->klass())->is_linked(),
           "must be initialized");
    // JavaThread._threadObj -> java.lang.Thread 对象
    // jvm中的JavaThread 与 java中的Thread对象建立关联
    set_threadObj(thread_oop());
    // 反向关联,将 C++ JavaThread 指针存入 Java Thread 对象的 eetop 字段
    java_lang_Thread::set_thread(thread_oop(), this);
...
protected:
    ...
    inline void do_oop_work(T *p) {
        ...
        if (obj == NULL) return;
        tty->print(INTPTR_FORMAT
        ": ", p2i(p));
        if (oopDesc::is_oop_or_null(obj)) {
            if (obj->is_objArray()) {
                tty->print_cr("valid objArray: "
                INTPTR_FORMAT, p2i(obj));
            } else {
                obj->print();
            }
        } else {
            tty->print_cr("invalid oop: "
            INTPTR_FORMAT, p2i(obj));
        }
        tty->cr();
    }
...
jint Threads::create_vm(JavaVMInitArgs *args, bool *canTryAgain) {
    ...
    main_thread->record_stack_base_and_size(); /* 记录栈基址和栈大小 */
    /**
            NMT = Native Memory Tracking，是 JVM 的内存追踪系统，用于：
               - 追踪 JVM 各部分的内存使用
               - 通过 jcmd <pid> VM.native_memory 查看内存报告
               - 诊断内存泄漏
            生存环境默认是不开启的,会有5-10% 的性能损耗
     */
    main_thread->register_thread_stack_with_NMT();/* 将线程栈注册到 NMT (Native Memory Tracking) 系统中 */
    main_thread->set_active_handles(
            JNIHandleBlock::allocate_block()); /* 为主线程分配JNI-Handle,用于在native代码中安全的引用Java对象*/ // forcus 将申请的block设置到JavaThread对象的_active_handles属性中去
    MACOS_AARCH64_ONLY(main_thread->init_wx());
    // forcus 将主线程(main_thread,这里指的是前面所说的子线程) 正式的 附加(attach) 到 os线程
    /*
     * case:
     *  - 创建OSThread对象
     *  - 设置main thread 的 信号相关(阻塞/非阻塞)
     */
    ...
    // forcus 在线程栈的低地址创建保护页(guard page) 当栈溢出时触发硬件异常，防止程序继续破坏内存。
    ...
    for (agent = Arguments::libraries(); agent != NULL; agent = next) {
        ...
        if (on_load_entry == NULL) {
            ...
            if (on_load_entry != NULL) {
                ...
                vm_exit_during_initialization("Could not find JVM_OnLoad or Agent_OnLoad function in the library",
                                              agent->name());
            }
        }
    }
...
void Threads::assert_all_threads_claimed() {
    ALL_JAVA_THREADS(p) {
        ...
        assert((thread_parity == _thread_claim_parity),
               "Thread "
        PTR_FORMAT
        " has incorrect parity %d != %d", p2i(p), thread_parity, _thread_claim_parity);
    }
    VMThread *vmt = VMThread::vm_thread();
    ...
    assert((thread_parity == _thread_claim_parity),
           "VMThread "
    PTR_FORMAT
    " has incorrect parity %d != %d", p2i(vmt), thread_parity, _thread_claim_parity);
}
...
st->
print(PTR_FORMAT, p2i(this_thread)
);
st->print(" ");
this_thread->
print_on_error(st, buf, buflen
);
st->
